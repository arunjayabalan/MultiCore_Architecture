...
#define BLOCK_SIZE IMAGE_SIZE/THREAD_SIZE
...
int main(int argc, char** args)
{
	...
	//Kernel invocation
	calc_fractal<<< BLOCK_SIZE, THREAD_SIZE>>>( 0.28, 0.008, fractal);
	

	//copying from Device to Host
	cudaMemcpy(&julia, fractal, (N)*sizeof(char), cudaMemcpyDeviceToHost) ;

	write_ppm(julia, 256, 256, "julia.ppm");	
